...
public class ProductoImpl extends Conexion implements ICRUD<Producto> {
    ...
    public void registrar(Producto producto) throws Exception {
        String sql = "insert into PRODUCTO (NOMPRO, MARPRO, CODFAM, DESPRO, PREPRO, VENPRO, CODPROV, ESTPRO) values (?,?,?,?,?,?,?,?)";
        try {
            ...
            ps.setString(6, formato.format(producto.getVENPRO()));
            ps.setString(7, producto.getCODPROV());
            ps.setString(8, "A");
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
    ...
    public void modificar(Producto producto) throws Exception {
        String sql = "update PRODUCTO set NOMPRO=?, MARPRO=?, CODFAM=?, DESPRO=?, PREPRO=?, VENPRO=?, CODPROV=?, ESTPRO=? where IDPRO=?"; //where IDPRO like?
        try {
            PreparedStatement ps = this.conectar().prepareStatement(sql);
            ps.setString(1, producto.getNOMPRO());
            ps.setString(2, producto.getMARPRO());
    ...
    public List listarTodos() throws Exception {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                producto.setCODPROV(rs.getString("CODPROV"));
...
//        String sql = "select top 10 (P.NOMPROV) as NOMPROV from PROVEEDOR P where NOMPROV like ?";
        String sql = "select top 52 NOMPROV as PROVEEDORDESC from PROVEEDOR where NOMPROV like ?";
        try {
            PreparedStatement ps = this.conectar().prepareCall(sql);
            ps.setString(1, "%" + consulta + "%");
            ResultSet rs = ps.executeQuery();
