...
//    }
    ...
    public void registrar(Producto producto) throws Exception {
        String sql = "insert into PRODUCTO (NOMPRO, MARPRO, FAMPRO, DESPRO, PREPRO, ESTPRO) values (?,?,?,?,?,?)";
        try {
...
//            ps.setDate(6, producto.getVencimiento());
    ...
    public void modificar(Producto producto) throws Exception {
        String sql = "update PRODUCTO set NOMPRO=?, MARPRO=?, FAMPRO=?, DESPRO=?, PREPRO=?, ESTPRO=? where IDPRO=? "; //where IDPRO like?
        try {
            PreparedStatement ps = this.conectar().prepareStatement(sql);
            ps.setString(1, producto.getNombre());
            ps.setString(2, producto.getMarca());
